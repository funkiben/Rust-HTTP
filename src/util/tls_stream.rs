...
pub struct TlsStream<S: Session, T: Stream>(StreamOwned<S, T>);

impl<S: Session, T: Stream> TlsStream<S, T> {
...
impl<S: Session, T: Stream> Read for TlsStream<S, T> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
        self.0.read(buf)
    }
}

impl<S: Session, T: Stream> Write for TlsStream<S, T> {
...
impl<S: Session, T: Stream> Drop for TlsStream<S, T> {
    fn drop(&mut self) {
